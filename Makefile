...
tag-message := A signed and notarized universal installer package for \`flac\` $(version), built with libiconv "$(libiconv_version).
tag-title := flac $(version) for macOS rev $(revision)
ver := $(version)$(rev)
...
$(TMP)/flac/build_shared/src/libFLAC/.libs/libFLAC.14.dylib : \
...
$(TMP)/installed-and-signed.stamp.txt : \
	...
	xcrun codesign \
		--sign "$(APP_SIGNING_ID)" \
		--options runtime \
		$(TMP)/flac/install/usr/local/lib/libFLAC.14.dylib
	xcrun codesign \
		--sign "$(APP_SIGNING_ID)" \
		--options runtime \
		$(TMP)/flac/install/usr/local/lib/libFLAC++.11.dylib
